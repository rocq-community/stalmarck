...
(** A well formed array has all the previous properties.
 The idea is that we want every non minimal elements to point by
 Ref to its minimal element and the minimal elment to contain by
 Class the elements pointing to it
